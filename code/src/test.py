...
def test(configs, checkpoint_path, exp_name, leave_out_users=None, enable_fusion=True, disable_print=True):
    ...
    model = LitModel_joint(configs, training_config=config_default, enable_fusion=enable_fusion, disable_print=disable_print)

    # Get trainer kwargs from the model to handle print disabling
    trainer_kwargs = model.get_trainer_kwargs()

    trainer = pl.Trainer(
        accelerator='auto',
        devices='auto',
        logger=False,  # Disable default logger to prevent lightning_logs creation
        enable_checkpointing=False,  # Disable checkpointing
        **trainer_kwargs  # Apply model-specific trainer settings
    )
